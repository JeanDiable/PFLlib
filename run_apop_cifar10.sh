...
# Configuration
DATASET=${2:-"Cifar10"}  # Fixed: Use Cifar10 to match actual data
NUM_CLIENTS=${3:-2}
CILRPC=${4:-30}  # Rounds per task (main parameter to adjust)
SEQUENCE_FILE=${1:-""}

# Task sequence configuration (adjusted for Cifar10)
NUM_TASKS_PER_CLIENT=5  # Reduced to fit in 10 classes
CLASSES_PER_TASK=2      # Use 2 classes per task to fit better
...
python main.py \
    ...
    -go "${DATASET}_${NUM_CLIENTS}c_${NUM_TASKS_PER_CLIENT}t_${CILRPC}rpt_orth2"
